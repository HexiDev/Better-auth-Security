          ...
          handler: createAuthMiddleware({}, async (ctx) => {
            const creation = (async () => {
              ...
              const checkData: {
                ...
                ? JSON.parse(atob(headers.get("X-Data") || ""))
                : null;

              if (!headers || !checkData) {
                return;
              ...
              let totalWeight =
                modules?.reduce(
                  (acc, module) => acc + (module.weight || 0),
                  0
                ) || 0;
              if (modules && modules.length > 0) {
                for (const module of modules) {
                  const lying = await module.isLying(
                    checkData.find((item) => item.id === module.id) ?? {}
                  );
                  if (!lying) {
                    items.push({
                      id: module.id,
      ...
      after: [
        {
          ...
          handler: createAuthMiddleware({}, async (ctx) => {
            ...
            if (!session) {
              ...
              session = await ctx.context.internalAdapter.findSession(
                decodeURIComponent(sessionCookieToken)
              );
            }
            ...
            if (fingerprintId) {
              ...
              let fingerprintRecord: FingerprintRecord | null =
              ...
              if (!fingerprintRecord)
                return console.error(
                  new Error(
                    "Fingerprint record not found after creation or update"
                  )
                );
