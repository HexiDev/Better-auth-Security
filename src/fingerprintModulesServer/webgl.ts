...
export default {
  ...
  async isLying(ctx) {
    ...
    const matched = expected.find((item) =>
      normalizedVendor.includes(item.vendor)
    );

    if (!matched) return true;

    const suspicious = matched.mustInclude.some(
      (keyword) => !normalizedRenderer.includes(keyword)
    );
